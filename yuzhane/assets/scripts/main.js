...
function tooltip() {
    ...
    const init = () => {
        mains.forEach(main => {
            ...
            if ((parameters.top - parameters.height - 50) < 0) {
    ...
    if (window.matchMedia("(max-width: 992px)").matches) {
        document.addEventListener('click', (event) => {
            if (event.target.closest('[data-tooltip="main"]')) {
                if (event.target.closest('[data-tooltip="icon"]')) {
                    const icon = event.target.closest('[data-tooltip="icon"]')
